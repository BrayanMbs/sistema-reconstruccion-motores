...
import { limitFromQuery, optionalDate, optionalText, pageFromQuery, requireText, requireUuid } from "../validators/common.validators";
...
export const getWorkOrderTimeline = async (request: Request, response: Response): Promise<void> => { response.json({ items: await service.timeline(requireUuid(request.params.id, "Orden")) }); };
